...
class DDPGAgent(object):
    ...
    def select_action(self, obs, is_exploring=False, return_embed=False):
        ...
        """
        if return_embed:
            action, embed_logits = self.policy(obs, return_embed_logits=True, explore_emb=is_exploring)
            embed = onehot_from_logits(embed_logits)
        else:
            action = self.policy(obs, explore_emb=is_exploring)
        if self.use_discrete_action:
            if is_exploring:
                ...
                action = differentiable_onehot_from_logits(action)
