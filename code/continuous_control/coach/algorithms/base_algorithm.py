...
class Algorithm(object):
    ...
    def update_coach(self, sample):
        ...
        if any(['Coach' in alg for alg in self.alg_types]):
            ...
            coach_embed = gumbel_softmax(coach_embed_logits, hard=True)  # this is the only exploration that the coach
            # has a varying temperature might be useful
    ...
    def init_from_save_dict(cls, save_dict):
        ...
        try:
            ...
            [save_dict['init_dict'].pop(key, None) for key in args_to_remove]
            instance = cls(**save_dict['init_dict'])
