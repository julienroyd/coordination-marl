...
def gumbel_softmax_sample(logits, temperature):
    ...
    # here the temparature is not related to probabilities, just to the final softmax to make it look like a one-hot!
...
def differentiable_onehot_from_logits(y):
    y = F.softmax(y, dim=-1)
    y_hard = onehot_from_logits(y)
    y = (y_hard - y).detach() + y
    return y
...
def multiply_gradient(parameters, coef):
