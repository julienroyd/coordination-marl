...
from utils.make_env import make_particle_env, make_football_env
...
def get_evaluation_args(overwritten_args=None):
    ...
    parser.add_argument("--last_model", default=True, type=parse_bool,
...
def evaluate(config):
    ...
    if '3v2football' == env_params['env_name']:

        if algorithm.feature_extractor == "identity":
            assert config.representation in ['simple37', 'simple115']
            n_stack = 1
        elif algorithm.feature_extractor == "convNet":
            assert config.representation == 'extracted'
            n_stack = 4
        else:
            raise NotImplementedError

        env = make_football_env(seed_dir=dir_manager.seed_dir,
                                dump_freq=0,
                                representation=config.representation,
                                render=config.render)  # no rendering during training

        obs_buffer = ObsBuffer(n_stack)
    else:
        env = make_particle_env(scenario_name=env_params['env_name'],
                                use_discrete_action=algorithm.use_discrete_action,
                                use_max_speed=env_params['use_max_speed'],
                                world_params=world_params)
        obs_buffer = ObsBuffer(1)


    if config.render and "football" not in env_params['env_name']:
        env.render()
    ...
    for ep_i in range(config.n_episodes):
        ...
        for t_i in range(config.episode_length):
            ...
            torch_obs = [Variable(torch.Tensor((agent_obs)), requires_grad=False).unsqueeze(0)
                         for agent_obs in obs_buffer.read()]
            ...
            if (config.render and "football" not in env_params['env_name']) or config.save_gifs:
