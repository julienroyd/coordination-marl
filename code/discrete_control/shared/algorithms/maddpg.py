...
from algorithms.algo_list import SHARED_ALGOS
...
class MADDPG(Algorithm):
    ...
    def __init__(self,
        ...
        if all([alg in SHARED_ALGOS for alg in alg_types]):
    ...
    def update_agent(self, sample, agent_i):
        ...
        if self.alg_types[agent_i] in ['SharedMADDPG', 'MADDPG']:
            ...
            if self.critic_concat_all_obs:
                target_vf_in = torch.cat((*next_obs, *all_target_actions), dim=1)  # TODO: WARNING: should probably feed that in feature_extractor
            else:
    ...
    def update(self, sample, train_recorder):
        ...
        for i in range(self.nagents):
            ...
            if self.alg_types[i] in SHARED_ALGOS:
                a_i = np.random.randint(self.nagents)
            else:
                a_i = i
            ...
            all_agents_actor_loss[0, i] = actor_loss
            all_agents_critic_loss[0, i] = critic_loss
    ...
    def init_from_env(cls,
        ...
        for action_space, observation_space, alg_type in zip(env.action_space, env.observation_space, alg_types):
            ...
            if alg_type in ['SharedMADDPG', 'MADDPG']:
                num_in_critic = 0
