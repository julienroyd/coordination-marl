    ...
    def update_agent(self, sample, agent_i):
        ...
        if self.alg_types[agent_i] in ['TeamMADDPG', 'MADDPG']:
            if self.use_discrete_action:  # one-hot encode action
                all_target_actions = [onehot_from_logits(pi(f_e(nobs)).view(-1, self.action_spaces[0], self.nagents)[:,:,i]) for i, (pi, f_e, nobs) in enumerate(zip(self.target_policies, self.f_es, next_obs))]
            else:
                all_target_actions = [pi(f_e(nobs)).view(-1, self.action_spaces[0], self.nagents)[:,:,i] for i, (pi, f_e, nobs) in enumerate(zip(self.target_policies, self.f_es, next_obs))]
        ...
        if self.use_discrete_action:
            ...
            pol_out_all_heads = curr_agent.policy(curr_agent.f_e(observations[agent_i])).view(-1, self.action_spaces[0], self.nagents)
        ...
        if self.alg_types[agent_i] in ['TeamMADDPG', 'MADDPG']:
            ...
            for i, pi, f_e, ob in zip(range(self.nagents), self.policies, self.f_es, observations):
                if i == agent_i:
                    ...
                    logits = pi(f_e(ob)).view(-1, self.action_spaces[0], self.nagents)[:, :, i]
                    all_pol_logits.append(logits)
                    all_pol_acs.append(onehot_from_logits(logits).detach())
                else:
                    all_pol_logits.append(None)
                    all_pol_acs.append(pi(f_e(ob)).detach().view(-1, self.action_spaces[0], self.nagents)[:,:,i])
    ...
    def create_train_recorder(self):
        return TrainingRecorder(
            stuff_to_record=['total_reward', 'actor_loss', 'critic_loss', 'agent_colors', 'eval_episodes',
                             'eval_total_reward_explore', 'eval_total_reward_exploit', 'ts_loss'])
