...
class DDPGAgent(object):
    ...
    def select_action(self, obs, is_exploring=False):
        ...
        raw_action = self.policy(self.f_e(obs)) # shape is (batch, n_agents*act_dim)
        ...
        if self.use_discrete_action:
            ...
            action  # we must remove the squeeze because we consider the batch dim as the env dim further
