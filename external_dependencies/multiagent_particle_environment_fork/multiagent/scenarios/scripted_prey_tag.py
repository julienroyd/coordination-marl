...
class Scenario(BaseScenario):
    ...
    def reset_world(self, world):
        ...
        for i, agent in enumerate(world.agents):
            agent.color = np.array([0.85, 0.35, 0.35]) if agent.adversary else np.array([0.35, 0.85, 0.35])
        ...
        while True:
            ...
            for agent_i in world.agents:
                if not agent_i.adversary:  # agent_i is an predator
                    continue
                for agent_j in world.agents:
                    if agent_j.adversary:  # agent_j is a prey
                        continue
    ...
    def agent_dense_reward(self, agent, world):
        assert not agent.adversary
        rew = 0
        agents = self.good_agents(world)
        adversaries = self.adversaries(world)
        for ag in agents:
            if (not self.individual_reward) or (ag is agent):
