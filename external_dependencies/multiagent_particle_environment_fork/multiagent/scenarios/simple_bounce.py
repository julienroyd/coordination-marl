...
class Scenario(BaseScenario):
    def make_world(self, episode_length=100, line_length=0.5, use_dense_rewards=False):
        ...
        world.lines = [ConnectionLine(world.agents[0], world.agents[1], max_length=line_length, hard=True, elasticity=2.)]
        # add landmarks
    ...
    def reset_world(self, world):
        ...
        for i, agent in enumerate(world.agents):
            agent.color = np.array([0.6, 0.3, 0.8])  # violet
        # random properties for landmarks
        for i, landmark in enumerate(world.landmarks):
            if landmark.role == 'bouncing-ball':
                landmark.color = np.array([0.8, 0.0, 0.0])  # red
    ...
    def rew_ball_in_target(self, world):
        ...
        if ball.has_bounced:
            if self.ball_target_will_intersect(ball, target):
                ...
                rew = 10.
                target.color = np.array([0.5, 0.8, 0.3])
                self.is_done = True
                return rew
    ...
    def rew_bounce(self, world):
        ...
        # # ball left by the bottom
        # if ball.state.p_pos[1] < -1. - ball.size:
        #     rew = 0.
        #     self.ball_has_bounced = True
        # # ball left by one of the sides
        # if ball.state.p_pos[0] < -1. - ball.size or ball.state.p_pos[0] > +1. + ball.size:
        #     raise ValueError('this should not happen')
        # # ball left by the top
        # if ball.state.p_pos[1] > +1. + ball.size:
        #     raise ValueError('this should not happen')
        ...
        if ball.has_bounced:
            ...
            # ball is heading towards the left or right side
