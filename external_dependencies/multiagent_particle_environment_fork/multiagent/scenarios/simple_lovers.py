...
class Scenario(BaseScenario):
    def make_world(self, n_agents=2, line_length=0.3, use_dense_rewards=False):
        world = World()
    ...
    def reset_world(self, world):
        ...
        colors = [np.array(cm(float(i) / float(len(world.agents)))[:3]) for i in range(len(world.agents))]
        ...
        # landmark colors
        for i, landmark in enumerate(world.landmarks):
            landmark.color = colors[i] - np.array([0.05, 0.05, 0.05])
